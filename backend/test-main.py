...
@app.get("/demo-cvs")
def get_demo_cvs():
    """[PUBLIC] Lấy danh sách 100 CV mẫu từ dataset"""
    return {
        "cvs": demo_cvs,
        "total": len(demo_cvs),
        "type": "demo"
...
def match_demo(request: DemoMatchRequest):
    ...
    if missing_skills and course_emb is not None:
        try:
            ...
            logger.info(f"🔍 Found {len(top_indices)} courses for {len(missing_skills)} missing skills")
            ...
            for i in top_indices:
                if i < len(courses):
                    ...
                    recommended_courses.append({
                        ...
                        "num_total_skills": len(course_skills),
                        "num_relevant_skills": len(relevant_skills)
                    })
        ...
        logger.info("✅ CV already has all required skills - no course recommendations needed")
...
def match_user_cv(
    ...
    """[PROTECTED] So sánh Job với CV của user"""
    
    job = next((j for j in jobs if j["job_id"] == job_id), None)
    if not job:
        raise HTTPException(404, "Công việc không tồn tại")
    
    cv = next((c for c in user_cvs 
              if c.get("cv_id") == cv_id and c.get("user_id") == current_user.id), None)
    if not cv:
        raise HTTPException(404, "CV không tồn tại hoặc không thuộc quyền sở hữu")
    ...
    if match_score >= 80:
        assessment = "Xuất sắc! CV của bạn rất phù hợp với công việc này."
        level = "excellent"
    elif match_score >= 60:
        assessment = "Tốt! Bạn có nền tảng vững, cần bổ sung một số kỹ năng."
        level = "good"
    elif match_score >= 40:
        assessment = "Khá! Bạn có một số kỹ năng cần thiết."
        level = "fair"
    else:
        assessment = "Cần cải thiện đáng kể các kỹ năng còn thiếu."
        level = "needs_improvement"
